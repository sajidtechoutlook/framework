<?php
	if( isset($_POST) && isset($_POST['login_form_submitted']) && $_POST['login_form_submitted'] == 'yes' ){
		$q = " select * from user where username = '".$rodb->mysql_real_escape_string($_REQUEST['username'])."' and password = '".md5($rodb->mysql_real_escape_string($_REQUEST['password']))."' and active = '1'";
		...
		if(isset($result[0]['uid'])){
			...
			if($result[0]['user_of'] != '0' && $result[0]['user_of'] != NULL){
			...
			$_SESSION['show_barcode'] = $result[0]['show_barcode'];
			$_SESSION['flag_packs_quantity'] = $result[0]['flag_packs_quantity'];
			$_SESSION['product_discount_flag'] = $result[0]['product_discount_flag'];
			
			$q = "select * from usermeta where user_id='".getBusinessId()."'";
			$usermeta = $dblink->getTableFromDB($q);
			if(isset($usermeta) && is_array($usermeta) && count($usermeta) > 0)
			foreach($usermeta as $key=>$value){
				$_SESSION[$value['meta_key']] = stripslashes($value['meta_value']);
			}
...
<html lang="en">
	...
	<head>
		...
		<meta charset="utf-8" />
		<title>YOUACCOUNTS | Login</title>
		<meta name="description" content="http://youaccounts.com">
		<meta name="viewport" content="http://youaccounts.com">
					...
					<div class="kt-grid__item kt-grid__item--order-tablet-and-mobile-2 kt-grid kt-grid--hor kt-login__aside" style="background-image: url(<?php themeUrl()?>assets/media//bg/bg-4.jpg);">
						...
						<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver">
							<div class="kt-grid__item kt-grid__item--middle">
								...
								<h4 class="kt-login__subtitle">Do business with our investment.</h4>
							</div>
						</div>
						<div class="kt-grid__item">
							<div class="kt-login__info">
								...
								<div class="kt-login__menu">
									<a href="<?php echo base_url()?>" class="kt-link">Privacy</a>
									<a href="<?php echo base_url()?>" class="kt-link">Legal</a>
									<a href="<?php echo base_url()?>" class="kt-link">Contact</a>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="kt-login__body">
							...
							<div class="kt-login__form">
								...
								<form class="kt-form" action="" id="kt_login_form1" method="post">
									<?php if(isset($_GET) && isset($_GET['error']) && $_GET['error']=='yes'){?><div class="alert alert-bold alert-solid-danger alert-dismissible" role="alert">			<div class="alert-text">Incorrect username or password. Please try again.</div>			<div class="alert-close">                <i class="flaticon2-cross kt-icon-sm" data-dismiss="alert"></i>            </div>		</div><?php }?>
									<input class="form-control" type="hidden" placeholder="login_form_submitted" name="login_form_submitted" autocomplete="off" value="yes">
									...
									<div class="kt-login__actions">
										...
										<button id="kt_login_signin_submit1" class="btn btn-primary btn-elevate kt-login__btn-primary" type="submit">Sign In</button>
									</div>
									<!--end::Action-->
								</form>
								<!--end::Form-->
