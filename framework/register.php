<?php 
	if(!empty($_POST['register']))
	{
		...
		if($_POST['phone'] == ''){
			...
			$q = "select phone from user where phone='".$rodb->mysql_real_escape_string($_POST['phone'])."'";
			$exists = $rodb->getCellFromDB($q);
			if($exists != ''){
		...
		if(empty($errors))
		{
			...
			$data = "(username, password, email, phone, active, membership, activation_key, registration_code, reg_date ) values 
			('".$rodb->mysql_real_escape_string($_POST['email'])."', '".md5($rodb->mysql_real_escape_string($_POST['password']))."', '".$rodb->mysql_real_escape_string($_POST['email'])."', '".$rodb->mysql_real_escape_string($_POST['phone'])."', '0', '0', '".$key."', '".$registration_code."', '".@date('Y-m-d H:i:s')."')";
			$err = $rodb->writeToDB($data, "user");
			$data_umeta = "(user_id, meta_key, meta_value) values 
			('".$err[1]."', 'new_bill_format', '1')";
			$err_umeta = $rodb->writeToDB($data_umeta, "usermeta");
			$subject = "Your registration at youaccounts.com is almost complete.";
			$message = "
			...
			?>
			<meta http-equiv="refresh" content="0;url=<?php echo getPageUrl('confirm_registration')?>&code=<?php echo $registration_code?>" />
			<?php
			exit;
		}
	}
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--desktop kt-grid--ver-desktop kt-grid--hor-tablet-and-mobile">
					...
					<div class="kt-grid__item kt-grid__item--fluid  kt-grid__item--order-tablet-and-mobile-1  kt-login__wrapper">
						...
						<div class="kt-login__head">
							...
							<a href="<?php echo getPageUrl('login');?>" class="kt-link kt-login__signup-link">Sign In!</a>
						</div>
							...
							<div class="kt-login__form">
								...
								<form class="kt-form" action="" id="kt_login_form1" method="post">
									...
									<div class="form-group">
										<input class="form-control" type="email" placeholder="Email" name="email" autocomplete="off" value="<?php echo (isset($_POST['email']))?$_POST['email']:''?>" required>
									</div>
									<div class="form-group">
										<input class="form-control" type="text" placeholder="Phone" name="phone" autocomplete="off" value="<?php echo (isset($_POST['phone']))?$_POST['phone']:''?>" required>
									</div>
									<div class="form-group">
										<input class="form-control" type="password" placeholder="Password" name="password" autocomplete="off" required>
									</div>
									<div class="form-group">
										<input class="form-control" type="password" placeholder="Confirm Password" name="c_password" autocomplete="off" required>
									</div>
									<!--begin::Action-->
									<div class="kt-login__actions">
										...
										<button id="kt_login_signin_submit1" class="btn btn-primary btn-elevate kt-login__btn-primary" type="submit" name="register" value="Sign Up">Sign Up</button>
									</div>
									<!--end::Action-->
								</form>
								<!--end::Form-->
